...
def _try_regular_cover_batch_commit_dream(
    *,
    x,
    x_fill,
    logits,
    ...
    for idx_t in indices:
        idx = int(idx_t.item())
        if idx in seen or x[0, idx].item() != mask_token_id:
            continue
        seen.add(idx)
        tok = x_fill[0, idx]
        ...
        # Avoid batch EOS because existing sequential code fills the suffix.
    ...
    try:
        ...
        if trace:
            print(f"[regular-cover] disabled for this DREAM step after error: {exc}")
        return []
    if len(selected) < regular_cover_min_batch():
        return []
    row_indices = torch.zeros((1, len(selected)), dtype=torch.long, device=x.device)
    transfer_index = torch.tensor([[c.index for c in selected]], dtype=torch.long, device=x.device)
    for c in selected:
        generated_words[c.index] = c.word
        x_fill[0, c.index] = c.token_id
    x[row_indices, transfer_index] = x_fill[row_indices, transfer_index]
    if trace:
        print(f"[regular-cover] committed {len(selected)} DREAM tokens")
    return selected
...
def _sample(
    ...
    for i in range(steps):
        ...
        # Fast path: select a regular-cover-compatible subset of this diffusion
        # step's top proposals and verify it exactly before committing.
        _regular_cover_committed = 0
        if constrain and number_transfer_tokens_overall >= regular_cover_min_batch() and regular_cover_batch_enabled():
            try:
                mask_logits_batch = logits[mask_index]
                if alg == "origin":
                    raise NotImplementedError("origin alg is not supported by regular-cover batch")
                elif alg == "maskgit_plus":
                    confidence_batch, x0_batch = sample_tokens(
                        mask_logits_batch,
                ...
                full_confidence_batch = torch.full_like(
                    x, -torch.inf, device=model.device, dtype=logits.dtype
                )
                full_confidence_batch[mask_index] = confidence_batch
                x_fill_batch = torch.zeros_like(x, device=model.device, dtype=torch.long) + mask_token_id
                x_fill_batch[mask_index] = x0_batch.clone()
                selected_batch = _try_regular_cover_batch_commit_dream(
                    x=x,
                    x_fill=x_fill_batch,
                    logits=logits,
                    full_confidence=full_confidence_batch,
                    tokenizer=tokenizer,
                    generated_words=generated_words,
                    prompt_len=prompt_len,
                    max_candidates=number_transfer_tokens_overall,
                ...
                if trace:
                    print(f"[regular-cover] DREAM batch fast path failed: {exc}")

        for _ in range(max(0, number_transfer_tokens_overall - _regular_cover_committed)):
            if complete:
                break
