...
from constrained_diffusion.regular_cover import (
    BatchCandidate,
    regular_cover_batch_enabled,
    regular_cover_min_batch,
    select_batch_with_regular_cover,
)
...
def _try_regular_cover_batch_commit_llada(
    *,
    x,
    x0,
    logits,
    ...
    if not regular_cover_batch_enabled() or max_candidates < regular_cover_min_batch():
        return []
    conf = confidence[0].clone()
    conf[block_end:] = -torch.inf
    k = min(int(max_candidates), int(torch.isfinite(conf).sum().item()))
    if k < regular_cover_min_batch():
        return []
    _, indices = torch.topk(conf, k=k)
    eos_token = tokenizer.special_tokens_map.get("eos_token")
    candidates = []
    seen = set()
    for idx_t in indices:
        idx = int(idx_t.item())
        if idx in seen or idx < prompt_shape_1 or x[0, idx].item() != mask_id:
            continue
        seen.add(idx)
        tok = x0[0, idx]
        if logits[0, idx, tok] == -torch.inf:
            continue
        word = tokenizer.decode(tok)
        # Avoid batch-committing EOS; EOS has suffix-filling side effects in the
        # existing decoder and is better handled by the sequential fallback.
        if word in (eos_token, "<|eot_id|>"):
            continue
        candidates.append(BatchCandidate(idx, int(tok.item()), word, float(conf[idx].item())))
    if len(candidates) < regular_cover_min_batch():
        return []
    try:
        selected = select_batch_with_regular_cover(
            words_full=generated_words,
        ...
        if trace:
            print(f"[regular-cover] disabled for this step after error: {exc}")
        return []
    if len(selected) < regular_cover_min_batch():
        return []
    transfer_index = torch.zeros_like(x, dtype=torch.bool, device=x.device)
    for cand in selected:
        generated_words[cand.index] = cand.word
        x0[0, cand.index] = cand.token_id
        transfer_index[0, cand.index] = True
    x[transfer_index] = x0[transfer_index]
    if trace:
        print(f"[regular-cover] committed {len(selected)} LLaDA tokens")
    return selected
...
def generate(
    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps):
            ...
            # Fast path: use a lexeme-level RTN-flattened regular cover to select
            # a simultaneously compatible subset of the top DLM proposals, then
            # verify that subset with the exact CFG oracle inside the selector.
            _regular_cover_committed = 0
            _n_transfer_this_step = int(num_transfer_tokens[0, i].item())
            if constrain and _n_transfer_this_step >= regular_cover_min_batch() and regular_cover_batch_enabled():
                mask_index = x == mask_id
                x0_batch = torch.argmax(logits_with_noise, dim=-1)
                if remasking == "low_confidence":
                    p_batch = F.softmax(logits.to(torch.float64), dim=-1)
                    x0_p_batch = torch.squeeze(
                        torch.gather(p_batch, dim=-1, index=torch.unsqueeze(x0_batch, -1)), -1
                    )
                elif remasking == "random":
                    x0_p_batch = torch.rand((x0_batch.shape[0], x0_batch.shape[1]), device=x0_batch.device)
                else:
                    x0_p_batch = torch.full_like(x0_batch, -torch.inf, dtype=logits.dtype)
                x0_p_batch[:, prompt.shape[1] + (num_block + 1) * block_length :] = -torch.inf
                x0_batch = torch.where(mask_index, x0_batch, x)
                confidence_batch = torch.where(mask_index, x0_p_batch, -torch.inf)
                selected_batch = _try_regular_cover_batch_commit_llada(
                    x=x,
                    x0=x0_batch,
                    logits=logits_with_noise,
                    confidence=confidence_batch,
                    tokenizer=tokenizer,
                    generated_words=generated_words,
                    prompt_len=prompt_len,
                    prompt_shape_1=prompt.shape[1],
                    block_end=prompt.shape[1] + (num_block + 1) * block_length,
                    max_candidates=_n_transfer_this_step,
                    mask_id=mask_id,
                    constraint_lang=constraint_lang,
                    lex_map=lex_map,
                    terminals=terminals,
                    prelex=prelex,
                    all_possible_lexings=all_possible_lexings,
                    inject_gap_size=inject_gap_size,
                    max_total_injections=max_total_injections,
                    subtokens=subtokens,
                    supertokens=supertokens,
                ...
                _regular_cover_committed = len(selected_batch)
                if _regular_cover_committed:
                    yield x, resamples, False
                    if EOS in generated_words and None not in generated_words[: generated_words.index(EOS)]:
                        complete = True
                        break

            # greedy constrained sampling for remaining tokens in this diffusion step
            for j in range(max(0, _n_transfer_this_step - _regular_cover_committed)):
                if complete:
                    # If the generation is complete, we can stop.
                    break
                mask_index = x == mask_id
