...
#[derive(Debug, Clone)]
pub struct TerminalGraphEdge {
    pub src: usize,
    pub dst: usize,
    /// None denotes an epsilon edge in the input graph; Some(i) denotes CFG terminal index i.
    pub terminal: Option<usize>,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
struct EarleyGraphItem {
    origin: usize,
    prod_id: usize,
    dot: usize,
    node: usize,
...
impl CFG {
    ...
    pub fn is_graph_intersection_empty(
        &self,
        num_nodes: usize,
        start_node: usize,
        accept_nodes: &[usize],
        ...
        if !*self._in_normal_form.get_or_init(|| self.is_normal_form()) {
            return self.to_normal_form().is_graph_intersection_empty(
                num_nodes,
                start_node,
                accept_nodes,
                edges,
        ...
        if num_nodes == 0 || start_node >= num_nodes {
            return true;
        }
        let accept_set: HashSet<usize> = accept_nodes.iter().copied().collect();
        if accept_set.is_empty() {
            return true;
        }

        let start_time = Instant::now();
        let mut edges_by_src: Vec<Vec<(Option<usize>, usize)>> = vec![Vec::new(); num_nodes];
        for edge in edges {
            if edge.src >= num_nodes || edge.dst >= num_nodes {
                continue;
            }
            if let Some(t) = edge.terminal {
                if t >= self.terminals.len() {
                    continue;
                }
            }
            edges_by_src[edge.src].push((edge.terminal, edge.dst));
        }

        let mut flat_prods: Vec<FlatProduction> = Vec::new();
        let mut prods_by_head: Vec<Vec<usize>> = vec![Vec::new(); self.productions.len()];
        for (head, bodies) in self.productions.iter().enumerate() {
            for body in bodies {
                let prod_id = flat_prods.len();
                flat_prods.push(FlatProduction {
                    head,
                    body: body.clone(),
                });
                prods_by_head[head].push(prod_id);
            }
        }
        if flat_prods.is_empty() {
            return true;
        }

        let mut agenda: Vec<EarleyGraphItem> = Vec::new();
        let mut seen: HashSet<EarleyGraphItem> = HashSet::new();
        // Items waiting for a completed non-terminal at their current graph node.
        let mut waiting: HashMap<(usize, usize), Vec<EarleyGraphItem>> = HashMap::new();
        // Completed spans indexed by (origin_node, nonterminal).
        let mut completed: HashMap<(usize, usize), Vec<usize>> = HashMap::new();
        // Avoid repeatedly predicting the same non-terminal at the same graph node.
        let mut predicted: HashSet<(usize, usize)> = HashSet::new();

        let mut push_item = |item: EarleyGraphItem,
                             agenda: &mut Vec<EarleyGraphItem>,
                             seen: &mut HashSet<EarleyGraphItem>| {
            if seen.insert(item) {
                agenda.push(item);
            }
        };

        // Seed the start symbol productions at the graph start node.
        for &prod_id in &prods_by_head[self.start_symbol] {
            push_item(
                EarleyGraphItem {
                    origin: start_node,
                    prod_id,
        ...
        while let Some(item) = agenda.pop() {
            ...
            if agenda.len() > 64_000_000 || steps > 64_000_000 {
                return true;
            }
            let prod = &flat_prods[item.prod_id];

            // Regular-language epsilon edges: move in the input graph without
            // advancing the grammar dot.
            if item.node < edges_by_src.len() {
                for (label, dst) in &edges_by_src[item.node] {
                    if label.is_none() {
                        push_item(
                            EarleyGraphItem { node: *dst, ..item },
                            &mut agenda,
                            &mut seen,
                        );
                    }
            ...
            if item.dot == prod.body.len() {
                // Completed non-terminal span.
                if prod.head == self.start_symbol
                    && item.origin == start_node
                    && accept_set.contains(&item.node)
                {
                    return false;
                }
                let comp_key = (item.origin, prod.head);
                let entry = completed.entry(comp_key).or_default();
                if !entry.contains(&item.node) {
                    entry.push(item.node);
                }
                if let Some(waiters) = waiting.get(&(item.origin, prod.head)).cloned() {
                    for waiter in waiters {
                        push_item(
                            EarleyGraphItem {
                                origin: waiter.origin,
                                prod_id: waiter.prod_id,
                                dot: waiter.dot + 1,
                                node: item.node,
                            },
                            &mut agenda,
            ...
            match prod.body[item.dot] {
                SymbolIndex::TerminalIndex(t) => {
                    if item.node < edges_by_src.len() {
                        for (label, dst) in &edges_by_src[item.node] {
                            if *label == Some(t) {
                                push_item(
                ...
                SymbolIndex::VariableIndex(v) => {
                    // Register this item as waiting for v at the current node.
                    waiting.entry((item.node, v)).or_default().push(item);
                    // Complete immediately against spans already known for v.
                    if let Some(done_nodes) = completed.get(&(item.node, v)).cloned() {
                    ...
                    // Predict productions of v at this graph node once.
                    if predicted.insert((item.node, v)) {
                        for &pred_prod_id in &prods_by_head[v] {
                            push_item(
                                EarleyGraphItem {
                                    origin: item.node,
                                    prod_id: pred_prod_id,
                                    dot: 0,
                                    node: item.node,
                                },
                                &mut agenda,
